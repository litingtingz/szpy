<template>
  <div class="page">
    ...
    ></Inquire>
    <div class="t-tab-top">
      <div class="tab-top-item tabImgActive_1 hand">采集APP用户列表</div>
    </div>
    <div class="page-box">
      <Table
        :page="page"
...
export default {
  ...
  data() {
    return {
      ...
      //数据展示
      //查询项
      cxData: this.$cdata.foreigners.cjapp.cx,
      facxData: this.$cdata.foreigners.cjapp.facx, //快速查询项
      //列表
      isEdit: true,
      lbData: this.$cdata.foreigners.cjapp.lb,
      lbBtn: this.$cdata.foreigners.cjapp.lbBtn,
      lbTab: this.$cdata.foreigners.cjapp.lbTab,
      ...
      dialogType: "",
      dialogData: {},
      labelData: this.$cdata.foreigners.cjapp.labelData,
      timer: "",
      onlySerial: "",
      resopd:{},
    };
  ...
  methods: {
    ...
    getISValue(n){
        ...
        this.$api.post(this.$api.aport5 + "/tjSysUser/getCurrLogonUsersubBureauCode", p, r => {
            ...
            if(this.resopd.subBureauCode){
             ...
             if(this.resopd.policeStationCode){
                if(n==1){
    ...
    informLcFnc(data) {
      ...
      if (data.key.dm == "policeStationCode") {
        if (data.data == "") {
          data.obj.areaCode = "";
        }
        if (data.obj.areaCode) {
          data.obj.areaCode = "";
        }
    ...
    getTable(flag, pdQ) {
      ...
      this.$api.post(
        this.$api.aport5 + "/znUserIntranet/listData",
        pdQ || this.cx,
        (r) => {
    ...
    informLcFncdia(data) {
      ...
      if(data.subBureauCode){
          this.$store.dispatch("aGetssdw", { bmbh: data.subBureauCode, type: "sspcs" });
      }
      if(data.policeStationCode){
           this.$store.dispatch("aGetssdw", { bmbh: data.policeStationCode, type: "zrq" });
      }
      ...
      if (this.dialogType == "del") {
        this.$confirm("是否删除本条数据？", "提示", {
          ...
          .then(() => {
            ...
            this.$api.post(
              this.$api.aport5 + "/znUserIntranet/delBatch",
              p,
              (r) => {
                if(r.data){
                    this.$message({
        ...
        this.isShowDialog = true;
               ...
               let p={
                     ...
                     'params':{
                         ...
                         'ustatus':data.data.ustatusBoolean==true?0:1
                     }
                 };
            this.$api.post(
